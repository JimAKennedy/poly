...
LaneGridView::LaneGridView(const VSTGUI::CRect& size,
...
void LaneGridView::draw(VSTGUI::CDrawContext* context) {
    ...
    static const char* kLaneNames[] = {
        ...
        "Tom Hi", "Tom Lo", "Ride", "Crash"};

    auto font = makeOwned<CFontDesc>("Arial", 11.0);

    for (int lane = 0; lane < kMaxLanes; ++lane) {
        double y = bounds.top + kPad + lane * (laneH + kPad);
        CRect laneRect(bounds.left + kPad, y,
                       bounds.left + kPad + laneW, y + laneH);
        ...
        CColor bg = active ? CColor(0x4A, 0x9E, 0xFF, 0x30)
                           : CColor(0x50, 0x50, 0x50, 0xFF);
        context->setFillColor(bg);
        context->drawRect(laneRect, kDrawFilled);

        CColor border = active ? CColor(0x4A, 0x9E, 0xFF, 0xCC)
                               : CColor(0x70, 0x70, 0x70, 0xFF);
        ...
        context->setFontColor(active ? CColor(0xFF, 0xFF, 0xFF, 0xFF)
                                     : CColor(0xA0, 0xA0, 0xA0, 0xFF));
        ...
        if (active && prob > 0.0) {
            ...
            CRect barRect(laneRect.left + 95, laneRect.top + 2,
                          laneRect.left + 95 + barW, laneRect.bottom - 2);
