...
Write-Host " The CI build + install is done. Now, in your VNC session, run these"
Write-Host " in ONE PowerShell window (the env var MUST be set before launch):"
Write-Host ""
Write-Host "   1. Set the CDP arg, then launch Cubase on the fixture:"
Write-Host "        `$env:WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS = '--remote-debugging-port=$CdpPort'"
Write-Host "        & 'C:\Program Files\Steinberg\Cubase 14\Cubase14.exe' ```"
Write-Host "            '$fixtureLine'"
Write-Host "   2. Wait for the Poly editor window to open."
Write-Host "   3. Signal 'go' by creating the sentinel file:"
Write-Host "        New-Item -ItemType File -Force '$GoFile'"
Write-Host ""
...
Write-Host " If the CDP check never turns green, the env var was not set in the"
Write-Host " launching shell before Cubase started — quit Cubase and redo step 1."
Write-Host "============================================================="
Write-Host ""
