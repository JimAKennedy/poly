...
Write-Host " The CI build + install is done. In your VNC session, COPY-PASTE this"
Write-Host " one command into a PowerShell window (works in both powershell.exe and"
Write-Host " pwsh — the pwsh -File form avoids the '&' call-operator parse error):"
Write-Host ""
Write-Host "     pwsh -File '$launchScript' -CdpPort $CdpPort"
Write-Host ""
...
Write-Host ""
Write-Host " Use that script — do NOT hand-type or write a local one-off: the env"
Write-Host " var MUST be set in the SAME shell before Cubase launches, or the CDP"
Write-Host " port never binds, and a local script drifts from version control."
Write-Host ""
