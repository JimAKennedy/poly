...
Write-Host "=== Cubase pids under inspection: $($cubasePids -join ', ') ==="

$fg = [Win32.Enum]::GetForegroundWindow()
Write-Host "=== foreground window handle right now: $fg ==="
...
Write-Host "`n=== top-level windows owned by the Cubase process tree ==="
...
# no caption).
Write-Host "=== likely Poly-editor / WebView2 candidates ==="
$rows | Where-Object {
    $_.Visible -and (
...
if ($listen) {
    ...
    try {
        ...
        Write-Host "  /json/version -> $($v.Browser)  (webSocketDebuggerUrl present: $([bool]$v.webSocketDebuggerUrl))"
    } catch {
        Write-Host "  /json/version request failed: $($_.Exception.Message)"
    }
} else {
    Write-Host "CDP LISTENER: NO (nothing listening on $CdpPort at snapshot time)"
}
