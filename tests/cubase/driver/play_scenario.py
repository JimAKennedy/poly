...
def wait_for_ready(inport, timeout_s, verbose=False):
    ...
    (surface never connected / never pinged) from "CC119 arrives but is filtered
    out" (channel or value mismatch) — the two failures produce the identical
    30s timeout otherwise.
    """
...
def run(args):
    ...
    with mido.open_input(in_name) as inport, mido.open_output(out_name) as outport:
        ...
        if not wait_for_ready(inport, args.ready_timeout, verbose=args.verbose):
            log(
                "error",
