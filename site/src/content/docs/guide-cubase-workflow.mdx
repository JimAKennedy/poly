---
title: "Cubase Workflow"
description: "Setup, MIDI routing, recording, scene chaining, and export workflow for Poly in Cubase."
---

import ListenFor from '../../components/ListenFor.astro';

Poly is a VST3 instrument that generates MIDI note events. It loads on a Cubase instrument track and outputs MIDI to drum instruments via Cubase's routing system. This guide covers the canonical workflow for setup, monitoring, recording, and export.

## Setup

### Load Poly on an Instrument Track

1. **Project > Add Track > Instrument**, then select **Poly**
...
### Route MIDI Output to a Drum Instrument

Poly generates MIDI that needs to reach a drum instrument (Battery, Groove Agent, Kontakt, or any GM-compatible kit).
...
Poly derives all timing from Cubase's transport. Everything is PPQ-based, so transport operations work without drift or accumulated-state issues:

- **Play** -- generates notes based on current PPQ position
- **Stop** -- output ceases immediately
- **Loop** -- seamless; Poly recalculates from the loop-start PPQ position
- **Tempo changes** -- take effect immediately (PPQ-based, not sample-based)
...
1. Open Poly's editor and click the **CHAIN** button in the scene bar (or use Cubase automation lanes)
...
### Transport Behavior with Chains
...
All macro knobs, scene controls, chain parameters, per-lane settings, and metric modulation values are automatable.
...
2. Use the preset selector dropdown in the header bar
3. Save as a VST3 preset (`.vstpreset` file) via Cubase's preset manager

### Load Presets

1. Use the preset dropdown in the plugin window
2. Or use Cubase's **MediaBay** to browse and load `.vstpreset` files

### State Versioning

Poly uses versioned state serialization. Presets saved with older versions load correctly in newer versions -- the plugin reads the version number and migrates the state forward.
...
| Notes cut off at loop point | Expected behavior -- Poly recalculates from loop start | This is correct deterministic behavior |
...
| Missing notes | `kMaxEventsPerBlock` exceeded | Reduce lanes or density for very dense passages |
...
The entire workflow is designed for iterative exploration: play, listen, adjust a parameter, listen again, capture what works. MIDI sends let you hear changes in real time without recording. When you find a groove you want to keep, arm the track and record a pass. The recorded MIDI is standard Cubase data -- edit it, layer it, export it. Poly stays live on its track, ready for the next experiment.
