...
flowchart TB
  subgraph host["Host (Cubase/VST3)"]
    ...
    midiout["Event MIDI out"]
  end

  subgraph engine["poly_engine<br/>(pure C++, no VST3, no audio-thread alloc)"]
    transport["Transport/Time → bar/beat/sub/lane-phase"]
    generator["Lane Generator → hit candidates per lane"]
    shaping["Dynamic Shaping → velocity/emphasis/env"]
    constraints["Constraint Layer → anchors / density caps"]
    scheduler["Output Scheduler → ordered NoteEvent list"]
  end

  plugin["poly_plugin<br/>AudioEffect/Processor<br/>+ EditController<br/>+ Web UI editor"]
  tests["poly_tests<br/>(off-host unit<br/>+ golden tests)"]
...
- `engine/` — **poly_engine**, a pure C++ static library
  - `include/` — public headers (types, engine, envelope, scene, and the rest)
  - `src/` — implementation
- `plugin/` — **poly_plugin**, the VST3 instrument
...
- `tests/` — Google Test suite, which runs without the VST3 SDK
  - `golden/` — deterministic reference output files
  - `host/` — off-host plugin and bridge integration tests
