...
![Poly web UI showing the Afrobeat 12/8 preset — header bar, five lane columns with Euclidean circles and step cells, and the master panel with macro sliders and convergence display](/screenshots/ui-overview.png)

### Header

Displays the plugin name **POLY**, a play button, BPM and time signature, a **preset selector dropdown**, and the patch **seed** with its history controls (previous, randomise, next). To the right: **A/B/Morph/Chain** scene controls, **Note Map**, the capture controls (**bars** and **Arm**), **Export**, and the bars-to-align counter. Click the preset dropdown to access Init (All Lanes) and all factory presets. Selecting any entry immediately loads all parameters.
...
Up to eight drum lanes, each representing an independent rhythmic voice. Each lane is a vertical column, and the **− Lane / + Lane** bar below the columns removes the last lane or adds one, up to eight.
...
The lane count (ID 300) and the seed (ID 301) are also automatable, but they are not hidden: the lane bar and the header's seed controls are their on-screen forms. See the [Parameter Reference](/appendix-parameters/) for the complete parameter list with IDs and ranges.
