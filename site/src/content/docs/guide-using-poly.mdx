...
Poly generates MIDI only — it produces no audio on its own. You need a drum sound source (Groove Agent, Battery, Kontakt, or any GM-compatible drum plugin) to hear the output.
...
The plugin window is 600 x 870 pixels, laid out as six horizontal sections from top to bottom.
...
Displays the plugin name **POLY** (blue), a **preset selector dropdown** in the center, and the version number (gray) on the right. Click the preset dropdown to access Init (All Lanes) and all 14 factory presets. Selecting any entry immediately loads all parameters.
...
- **Active toggle** — click the lane name to enable or disable it. Active lanes show a blue-tinted background; inactive lanes are dark gray.
- **Probability slider** — click or drag the horizontal blue bar to set the probability (0–100%) that each note triggers. Full bar = always plays; shorter bar = notes are randomly skipped.
- **Phase indicator** — the colored circle on the right (L1–L8) shows a rotating dot indicating the lane's current position within its rhythmic cycle. Each lane can have a different cycle length, so they rotate at different speeds.

### Scene Bar

A horizontal control strip between the lane grid and macros.
...
- **Morph slider** — when Morph is selected, drag to blend between Scene A (left) and Scene B (right). Disabled when A or B is selected directly.
- **CHAIN button** — opens the chain configuration popover (see [Scene Chaining](#scene-chaining)).
...
Click and drag up/down on any knob to adjust. Start with **Density** and **Swing** — they have the most immediate impact on the feel.

### Velocity Display

Eight vertical bar graphs (one per lane, L1–L8) showing the real-time velocity of the most recent note triggered. Green bars pulse when notes fire during playback. This section is read-only.

### Visualization

Two real-time displays at the bottom of the plugin window.

**Envelope Panel (left):** Shows envelope and automation curves for all active lanes overlaid in a single plot. Each lane uses a distinct color (L1 blue, L2 orange, L3 green, L4 red, L5 purple, L6 teal, L7 dark orange, L8 light blue). A vertical line moves across the plot showing the current phase position, with a colored dot following along each curve.

**Phase Alignment Panel (right):** Concentric rings, one per active lane. A colored dot orbits each ring at the lane's cycle rate. When dots align vertically (top), those lanes are "in phase" — their downbeats coincide. When dots scatter, the polymetric pattern is at maximum divergence. This is the key visualization for Poly's core concept: independent rhythmic cycles drifting in and out of alignment.
...
See the [Parameter Reference](/poly/appendix-parameters/) for the complete parameter list with IDs and ranges.
...
### Multi-Channel Routing in Cubase
...
4. Each track receives only its assigned lanes' notes — no cross-talk between instruments
...
- **Mix** — each pass adds to the same part (layered takes)
...
After recording, the MIDI part is standard Cubase data: open in the Key Editor or Drum Editor, quantize, transpose, or process with any MIDI function. The recorded MIDI is fully independent of Poly.
