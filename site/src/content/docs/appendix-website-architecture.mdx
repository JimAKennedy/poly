...
`base: '/poly'` is the first thing that has to be right on this project: GitHub Pages serves the site under `https://jimakennedy.github.io/poly/`, and every asset URL (samples, WASM, presets JSON) resolves against `import.meta.env.BASE_URL` so nothing 404s under the subpath.
