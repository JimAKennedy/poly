...
The principle is mathematical: if polos plays hits at positions {1, 3, 5, 7} within an 8-step cycle, sangsih plays at positions {2, 4, 6, 8}. The two patterns are complementary — their union covers every beat. Strict complementation like this is the textbook idealization; in practice the two players also strike together at structural tones — cadence points, phrase joins, and *angsel* breaks — and a pair that never overlaps sounds mechanical. Neither part makes musical sense alone. Together they are seamless.
