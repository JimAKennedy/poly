...
{/* citation-tier-ok: a community-organisation introduction standing in for the kotekan literature. fr-vitale-1990, "Kotekan: The Technique of Interlocking Parts in Balinese Music", is this exact subject and is already in Further Reading. M006/B03 owns the upgrade. */}
Kotekan is the Balinese technique of rhythmic interlocking. Two musicians — called *polos* and *sangsih* — each play a pattern that is incomplete on its own.<sup>[20](/appendix-references/#ref-20)</sup> Polos plays certain beats. Sangsih plays the complementary beats — exactly the ones polos leaves silent. Together they produce a single continuous melody at a speed no individual player could sustain.

The principle is mathematical: if polos plays hits at positions {1, 3, 5, 7} within an 8-step cycle, sangsih plays at positions {2, 4, 6, 8}. The two patterns are complementary — their union covers every beat. Strict complementation like this is the textbook idealization; in practice the two players also strike together at structural tones — cadence points, phrase joins, and *angsel* breaks — and a pair that never overlaps sounds mechanical (see [theory-gamelan](/theory-gamelan/) Rule 4). Neither part makes musical sense alone. Together they are seamless.
...
{/* patch-divergence-ok: gam-pokok-layer — M004/S03 adds the pokok layer; ledger row F41 owns
    it. theory-gamelan Rule 6 makes the kotekan an ornamentation system over a core melody. */}
{/* patch-divergence-ok: gam-structural-overlap — M004/S03 adds structural overlap at the cycle
    boundary; ledger row F41 owns it. Sangsih is Kotekan L1, the strict complement of polos, so
    the pair intersect nowhere by construction and Rule 4 calls that mechanical. */}
<PolyPatch title="Balinese Kotekan Interlocking" preset="Balinese Kotekan">
...
<PolyPreviewCard preset="Balinese Kotekan" caption="Balinese kotekan interlocking — polos and sangsih with colotomic bass punctuation" />
...
Where Balinese kotekan operates at the level of individual beats, Javanese gamelan organises time at a much larger scale through *colotomic structure*.<sup>[18](/appendix-references/#ref-18)</sup> The word comes from the Greek *kolon* (member) — it describes how a long temporal cycle is divided into nested, hierarchical parts by different instruments.
...
<PolyPatch title="Javanese Colotomic Nesting" preset="Javanese Colotomic">
...
<PolyPreviewCard preset="Javanese Colotomic" caption="Javanese colotomic nesting — ketuk, kempul, kenong, and gong at 4:8:16:32 cycle ratios" />
