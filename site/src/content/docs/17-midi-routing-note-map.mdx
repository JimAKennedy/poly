...
Poly numbers channels 1-16, the same way Cubase and every other DAW does. The
underlying `midiChannel` parameter stores the MIDI *wire* value, which is
0-15 with `-1` reserved for Auto, so a lane displayed as Channel 1 is stored as
`0`. That distinction only matters if you are reading a saved patch or the
parameter automation directly; everywhere Poly shows you a channel, it shows
the number your DAW shows.
