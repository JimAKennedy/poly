...
| F07 | 2.1.2, §5.9 | Ch 2: Ewe ensemble music has been organised this way "for centuries." Unsourced; fieldwork documentation begins mid-20th century. | `P1` | `reframe` | `02-sub-saharan-africa.mdx` — the chapter opening paragraph; `appendix-references.mdx` ref-6 | M001/S05 | Case S05-F07 in `site/tests/theory-audit-claims.test.mjs` forbids unhedged multi-century constructions (`for centuries`, `for hundreds of years`) in Ch 2 and pins the claim to the documentary record and its two Tier-A sources: Jones (1959) — `ref-6`, which this slice repointed from *Wikipedia, "Ewe Drumming"* to the two-volume study built largely on Ewe material — and Locke (1982) — `fr-locke-1982`, *Ethnomusicology* 26(2), on Southern Ewe dance drumming specifically. An intermediate draft hedged to "understood within the tradition as considerably older than that record"; that clause was dropped rather than cited, no source meeting this guide's bar having been found for Ewe musical time depth | `done` |
| F08 | 2.1.1 | Ch 2 calls E(7,12) "the single most important timeline pattern in sub-Saharan music." Toussaint (2005) says "most commonly used" — descriptive, not evaluative. | `P1` | `reframe` | `02-sub-saharan-africa.mdx` — the gankogui / E(7,12) paragraph; `appendix-euclidean-reference.mdx` — the E(7,12) prose under the 12-step table | M001/S05 | Cases S05-F08 and S05-F08-appendix in `site/tests/theory-audit-claims.test.mjs` forbid `single most important` and require Toussaint's descriptive phrasing plus the stated descriptive-not-evaluative distinction. The claim appears twice — the audit cited only Ch 2, but `appendix-euclidean-reference.mdx` repeats it below the generated E(12) table — so both sites are corrected and locked, and Ch 2's footnote moves from ref-4 (Wikipedia) to ref-1 (Toussaint), the source the sentence names | `done` |
...
| S05 | Ch 2: bound the "centuries" claim to the documentary record and re-source it — `ref-6` moves from *Wikipedia, "Ewe Drumming"* to Jones (1959), with Locke (1982) cited inline. Restore Toussaint's descriptive phrasing for E(7,12) and cite Toussaint (ref-1) rather than ref-4 for it. The E(7,12) superlative is corrected at both of its sites — the audit cited only Ch 2, but `appendix-euclidean-reference.mdx` repeats the sentence. | F07, F08 |
