...
| ID | Slice | Fix commit(s) | Notes |
|----|-------|---------------|-------|
| D1 | M047 S01 | `bd1ef37` | Chapter 3 rewrote 'Rotation' and 'Rumba vs Son' sections to correctly identify son clave (gaps 3-3-4-2-4) and rumba clave (gaps 3-4-3-2-4) as non-Euclidean, with the arithmetic reason (E(5,16)'s gap sequence is 3-3-4-3-3 — no 2-gap possible). Rotation lesson now demonstrated on E(3,8) tresillo (which really is Euclidean). E(5,16) redirected to bossa nova with cross-link to chapter 10. New Euclidean-seed-then-adjust workflow paragraph teaches users to start from E(5,16), flip Timeline mode, and shift one step (13→12) to reach the exact son clave. `site/src/content/docs/03-afro-cuban.mdx:34-77`. |
| D2 | M047 S01 | `bd1ef37` | E(5,16) row corrected: grouping `3+3+2+4+3+1` (6 intervals for 5 onsets, arithmetically impossible) → `3+3+4+3+3` (5 gaps summing to 16); Traditional Association `Bossa nova bass / son clave` → `Bossa nova bass`. Accompanying prose rewrote 'is the bossa nova bass pattern and the son clave' to correctly identify E(5,16) as bossa nova only, with cross-links to chapter 3 (son clave analysis) and chapter 10 (bossa nova reading). Toussaint citation preserved via existing ref-42. `site/src/content/docs/appendix-euclidean-reference.mdx:114-120`. |
| D3 | M047 S02 | `e293fc6` | Chapter 8 (`08-minimalism.mdx:18-20`) now prints Reich's *Clapping Music* as `x x x . x x . x . x x .` (8 claps at positions 0, 1, 2, 4, 5, 7, 9, 10; gap sequence 1-1-2-1-2-2-1-2). Follow-on paragraph names the close-but-not-equal relationship to E(8,12) (which is `x . x x . x x . x x . x` with gap sequence 2-1-2-1-2-1-2-1) and gives the arithmetic reason: Reich's opening three-consecutive-claps at positions 0-1-2 breaks Bjorklund's strict alternation, which no E(k,n) family with n=12 produces at any rotation. E(5,12) phasing demonstration in the following section unchanged (was correct). |
