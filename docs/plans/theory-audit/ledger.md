...
| B18 | **Two of M007/S01's not-checkable verdicts were wrong, found after M007 merged.** `theory-funk-soul` Rule 6 was excused as "a fixed micro-offset field in milliseconds, which the lane table does not report" — the table has a `Timing` column reporting exactly that, and the patch demonstrates the rule precisely (kick +3ms behind, snare −2ms ahead, hat 0ms on top). `theory-jazz` Rule 6 was excused as "the table holds one swing value" — it holds one per lane, which is what "voices' ratios differ slightly" is about. M007/S02 audited the **checkable** verdicts against the tables and flipped six; the reverse audit was never run, and a false not-checkable verdict is invisible because there is no case to register and nothing to fail | `P1` | `correct` | `site/tests/theory-patch-conformance.test.mjs` | Both rules carry cases proved to fail, and a new case asserts that any verdict excusing itself by naming an absent column names one that is genuinely absent | `done` |
